...
export function calculateAggregates(config: EnhancedGridConfig) {
  // filter only for the visible values
  const cellDatas = config.selectedCellDatas.filter(
    (cellData) => !cellData.hidden
  );

  // sum
  config.aggregates.sum = cellDatas.reduce(
  ...
  // count
  config.aggregates.count = cellDatas.filter(
